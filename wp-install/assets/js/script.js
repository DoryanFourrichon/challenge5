$(document).ready(function() {
	...
	if ( typeof data.activate_plugins !='undefined' ) {
        ...
        // $('#activate_plugins').val( data.activate_plugins.join(';') );

        const free_plugins = data.plugins,
            premium_plugins = data.premium_plugins,
            unsorted_plugins = free_plugins.concat(premium_plugins),
            plugins = unsorted_plugins.sort();

        $('#plugins').val( free_plugins.join(';') );
        $('#premium_plugins').val( premium_plugins.join(';') );
        ...
        $(plugins).each(function(index) {
            ...
            var checked = '';
            if (data.activate_plugins.includes(plugin_name))
                checked = ' checked';

            var plugins_keys = '<label><input id="plugin_' + plugin_name + '" type="checkbox" name="activate_plugin[]" value="' + index + '" data-slug="' + plugin_name + '" ' + checked + ' /> ' + plugin_name.replace(/-/gi, ' ') + '</label><br/>'
            $('.activate_plugins').append(plugins_keys);
	...
	$('#submit').click( function() {
		...
		if ( ! errors ) {
            ...
            /**
             *  Set input with active plugins slugs to activate them.
             */
            var $active_plugins = $('.activate_plugins');
            var $activate_plugins = $active_plugins.find('input:checked');
            if ($activate_plugins.length) {

                var plugins_slugs = [];

                $activate_plugins.each(function(index) {
                    var $active_plugin = $(this);
                    var plugin_slug = $active_plugin.data('slug');
                    plugins_slugs.push(plugin_slug);
                });

                var plugins_slugs_str = plugins_slugs.join(';');

                $active_plugins.append('<input type="hidden" name="activate_plugins" id="activate_plugins" value="' + plugins_slugs_str + '" />');
